...
async function handleLoginSubmit(e) {
    ...
    try {
        ...
        if (response.ok && data && data.success && data.user) {
            ...
            showToastNotification("Login Successful", `Welcome back, ${data.user.first_name || data.user.username}!`, "success");
            ...
            showToastNotification("Login Failed", errorMsg, "danger");
        }
    } catch (err) {
        // Fallback retry check
        const reconnected = await checkBackendHealth();
        if (!reconnected) {
            showToastNotification("Server Disconnected", "Unable to connect to the backend server. Please start the server using 'npm run dev' or 'start.bat'.", "danger");
        } else {
            showToastNotification("Connection Restored", "Connection restored! Please click Sign In again.", "info");
        }
    } finally {
        if (submitBtn) {
