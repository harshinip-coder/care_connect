...
def evaluate_emergency_timeouts(emergency):
    ...
    If the active PENDING stage has timed out, marks all notifications for that stage as NO_RESPONSE
    and advances escalation chain.
    """
    if emergency.status in ['RESPONDING', 'ACKNOWLEDGED', 'IN_PROGRESS', 'RESOLVED', 'CANCELLED', 'ESCALATED']:
        return emergency

    pending_notifs = emergency.notifications.filter(status='PENDING')
    now = timezone.now()

    timed_out_notifs = [n for n in pending_notifs if n.timeout_at and now >= n.timeout_at]
    if timed_out_notifs:
        role_timed_out = timed_out_notifs[0].recipient_role
        emergency.notifications.filter(recipient_role=role_timed_out, status='PENDING').update(status='NO_RESPONSE', responded_at=now)

        log_audit(
            emergency,
            f"{role_timed_out}_TIMEOUT",
            details=f"{role_timed_out} stage response timeout ({get_sos_timeout()}s)"
        )
...
def advance_escalation_chain(emergency):
    ...
    if current_level == 'NONE':
        ...
        next_level = 'SECURITY'
    elif current_level in ['SECURITY', 'EMERGENCY_CONTACT']:
    ...
    if next_level == 'PRIMARY_GUARDIAN':
        ...
        if not society_users.exists():
            log_audit(emergency, 'NO_SOCIETY_MEMBERS_AVAILABLE', details=f"No Society Members in '{society_name}'. Auto-escalating to Security.")
            return advance_escalation_chain(emergency)
        ...
        emergency.status = 'NOTIFYING_SECURITY'
        emergency.active_escalation_level = 'SECURITY'
        emergency.current_stage_start = now
        emergency.save()

        if not security_users.exists():
            log_audit(emergency, 'NO_SECURITY_AVAILABLE', details=f"No Security Personnel in '{society_name}'. Auto-escalating to Volunteers.")
            return advance_escalation_chain(emergency)
        ...
        for sec_user in security_users:
            EmergencyNotification.objects.create(
                ...
                status='PENDING',
                title=f'🚨 URGENT SECURITY ALERT — {society_name}',
                message=f"SOS ID #{emergency.id}\nResident: {res_name}\nSociety: {society_name}\nLocation: {emergency.location_address}\nType: {emergency.emergency_type}",
                timeout_at=timeout_at
            )
            ...
            Notification.objects.create(
                receiver=sec_user,
                emergency=emergency,
                title="🚨 Urgent Security SOS Alert",
                message=f"Immediate response requested for {res_name} in {society_name} at {emergency.location_address}!"
            )

        log_audit(emergency, 'SECURITY_NOTIFIED', details=f"Security Personnel of '{society_name}' notified ({security_users.count()} security). Timeout in {timeout_seconds}s")
        ...
        if not volunteer_users.exists():
            log_audit(emergency, 'NO_VOLUNTEERS_AVAILABLE', details=f"No Volunteers in '{society_name}'. Auto-escalating to Admin.")
            return advance_escalation_chain(emergency)
...
def process_response(emergency_id, user, action):
    ...
    with transaction.atomic():
        ...
        if action in ['ACCEPT', 'RESPONDING', 'REACHED']:
            ...
            if active_notif:
                role_declined = active_notif.recipient_role
                emergency.notifications.filter(recipient_role=role_declined, status='PENDING').update(status='DECLINED', responded_at=now)
